...
function validateAndUploadCSV(file, uploadStatus, uploadResult, showNotification) {
    ...
    reader.onload = function(e) {
        ...
        if (errors.length) {
            ...
            uploadResult.textContent = `Validation failed: ${errors.length} errors`;
            uploadResult.className = 'text-xs text-center text-red-600';
            uploadResult.classList.remove('hidden');
            showNotification(`CSV validation failed: ${errors.slice(0, 3).join(', ')}${errors.length > 3 ? '...' : ''}`, 'danger');
            return;
        }
        ...
        .then(data => {
            uploadStatus.classList.add('hidden');
            uploadResult.textContent = `Upload successful! Processed ${deduplicatedRows.length} rows`;
            uploadResult.className = 'text-xs text-center text-green-600';
            uploadResult.classList.remove('hidden');
            showNotification(`CSV uploaded successfully: ${deduplicatedRows.length} records${duplicateCount ? `, ${duplicateCount} duplicates removed` : ''}`, 'success');
        })
        .catch(error => {
            uploadStatus.classList.add('hidden');
